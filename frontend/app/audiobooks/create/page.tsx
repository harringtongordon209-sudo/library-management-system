...
export default function CreateAudiobookPage() {
    ...
    const [createdAudiobook, setCreatedAudiobook] = useState<AudioBookResponse | null>(null);
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await fetch(`/api/titles/${selectedTitleId}/Audiobook`, {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify({
                    narrator: narrator, // Using "narrator" exactly as specified
            ...
            if (response.ok) {
                const data: AudioBookResponse = await response.json();
                setCreatedAudiobook(data);
            } else {
                const errData = await response.json();
                setError(errData.message || "Failed to add the audiobook. Please try again.");
            }
        } catch (err) {
    ...
    if (createdAudiobook) {
        const selectedTitle = titles.find(
            (t) => t.title_id === createdAudiobook.title_id
        );
            ...
            <div className="max-w-2xl mx-auto p-6">
                <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
                    ...
                    <div className="space-y-3 mb-6">
                        ...
                        <p>
                            <span className="font-semibold text-gray-700">Narrator:</span> {createdAudiobook.narrator}
                        </p>
                        <p>
                            <span className="font-semibold text-gray-700">Runtime:</span> {createdAudiobook.runtime}
                        </p>
                        <div>
                            ...
                            <ul className="list-disc list-inside mt-2 bg-gray-50 border border-gray-200 rounded-md p-3 space-y-1 font-mono text-sm">
                                {createdAudiobook.created_serial_numbers.map((serial) => (
    ...
    return (
        <div className="max-w-2xl mx-auto p-6">
            <h1 className="text-2xl font-bold mb-6">Add a New Audiobook</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="pt-4">
                    ...
                    >
                        {isSubmitting ? "Saving..." : "Add Audiobook"}
                    </button>
                </div>
            </form>
        </div>
